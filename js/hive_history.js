...
// ═══════════════════════════════════════════════════════
// HIVE HISTORY — inspections, feeding, treatments, harvests
// ═══════════════════════════════════════════════════════
function openHiveHistory(hiveId) {
  ...
  h += '<div style="font-size:12px;color:var(--txt2);margin-bottom:14px;line-height:1.45">Full timeline for this hive — same categories as the Inspections tabs.</div>';
  ...
  if (feeds.length) {
    feeds.forEach(function(f) {
      var ftype = typeof feedTypeDisplay === 'function' ? feedTypeDisplay(f) : (f.feedType || f.feed_type || 'Feeding');
      var amt = (f.amount != null && f.amount !== '' && !isNaN(parseFloat(f.amount))) ? parseFloat(f.amount) + ' ' + esc(f.unit || '') : '';
      var supLine = typeof supplementDisplayLine === 'function' ? supplementDisplayLine(f) : (f.supplement && f.supplement !== 'None' ? f.supplement : '');
      ...
      h += '<div class="harv-row card" style="margin-bottom:8px"><div class="harv-ico" style="font-size:18px">🥣</div><div class="harv-info"><div class="harv-name">' + fmtDate(f.date) + '</div><div class="harv-meta">' + esc(ftype) + (amt ? ' · ' + amt : '') + supPart + '</div>' + notePart + '</div></div>';
    });
