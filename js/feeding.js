...
function openFeedingModal(item) {
  ...
  h += '<div class="fg"><label>Date</label><input id="f-feed-date" type="date" value="' + (edit ? (item.date || today) : today) + '"></div>';
  ...
  h += '<div class="row2">';
  h += '<div class="fg"><label>Amount</label><input id="f-feed-amount" type="number" step="0.1" min="0" value="' + esc(item ? (item.amount || '') : '') + '" placeholder="e.g. 1"></div>';
  ...
  h += '<button class="btn btn-p" onclick="saveFeeding(\'' + (edit ? item.id : '') + '\',' + (edit ? 1 : 0) + ')">' + (edit ? 'Save Changes' : 'Log Feeding 🍯') + '</button>';
  ...
  // Show/hide Other fields when pills change
  setTimeout(function() {
...
async function saveFeeding(eid, isEdit) {
  ...
  if (isEdit) {
    ...
    if (idx >= 0) {
      DATA.feedings[idx] = Object.assign(DATA.feedings[idx], {
        ...obj,
        hiveId: obj.hive_id,
